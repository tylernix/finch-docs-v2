...
As with our automated integrations, your user will need to go through [Finch Connect](/docs/embed-finch-connect). However, instead of our standard login flow, the user will need to follow a set of instructions displayed on Finch Connect to manually connect your application to their employment system.
...
Launch [Finch Connect](/docs/embed-finch-connect) with `manual=true` so your user can view all the Assisted API providers Finch supports. After granting your application access, you can retrieve an authorization `code` and exchange it for an `access_token` as usual.
